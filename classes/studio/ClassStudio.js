//Declare a class called CrewCandidate with a constructor that takes three parameters—name, mass, and scores. Note that scores will be an array of test results.

class CrewCandidate {
    constructor (name, mass, scores = []){
    this.name = name;
    this.mass = mass;
    this.scores = scores
    }
    addScore(newScore) {
        this.scores.push(newScore)
    }  
    average(){
        let totalScore = 0;
        for ( let i =0; i < this.scores.length; i++) {
            totalScore += this.scores[i];
        }
        return Math.round(totalScore/this.scores.length * 10)/10
    };
    status() {
        let answer = ''
        if(this.average() < 70) {
            answer = 'Rejected'
        } else if ( this.average() <= 79 && this.average() >= 70) {
            answer = 'Probationary'
        } else if (this.average() <= 89 && this.average() >= 80) {
            answer = 'Reserve'
        } else { answer = 'Accepted'}
        return `${this.name} earned an average test score of ${this.average()}% and has a status of ${answer}`;
    }
    checkAccepted () {
        let counter = 0;
        while (this.average() < 90) {
            this.addScore(100);
            counter += 1
        }
        return counter;
    }
    
};

let bear = new CrewCandidate('Bubba Bear', 135, [88, 85, 90]);
let maltese = new CrewCandidate('Merry Maltese', 1.5, [93,88, 97]);
let gator = new CrewCandidate('Glad Gator', 225, [75,78, 62])


console.log(typeof bear.average())

console.log(bear.status())
console.log(maltese.status())
console.log(gator.status())
//Add methods for adding scores, averaging scores and determining candidate status as described in the studio activity.
console.log(maltese.checkAccepted())


//Part 4 - Use the methods to boost Glad Gator’s status to Reserve or higher. How many tests will it take to reach Reserve status? How many to reach Accepted? Remember, scores cannot exceed 100%.